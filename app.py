...
@st.cache_resource
def load_pickle():
    model = pickle.load(open('ada_hyper_f2_2.pkl', 'rb'))
    return model
...
#######################################################
# Function Name: get_devped_distances
# Description  : Compute distance from location based on current location
#######################################################
def get_devped_distances(data, seed):
...
# Description  : read YouTube Video Pool
#######################################################
def create_folium_map(data, seed):
...
# Description  : download Results
#######################################################
def process_location(current_loc):
    if current_loc != "":
        geolocator = Nominatim(user_agent="DSF C12 G3 Capstone")
        getLoc = geolocator.geocode(current_loc)

        if hasattr(getLoc, 'address'):
            #st.info(f"Found address: {getLoc.address}")
            reco_df = get_devped_distances(devpedia_df, getLoc)

            if len(reco_df) == 0:
                st.info(f"No Developmental Pediatrician Found")
            else:

                st.dataframe(reco_df[['Developmental Pediatrician','Address','Contact']], hide_index=True)
                create_folium_map(reco_df, getLoc)
        else:
...
if st.session_state.page_no == 0:
    ...
    result_tab = st.tabs(["Results", "Recommendation", "Parenting Tips", "Locate Doctors"])

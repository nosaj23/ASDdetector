...
st.write("""    
    **Coach Nadia** utilizes the Q-CHAT-10 (Quantitative Checklist for Autism in Toddlers) to screen toddlers for signs of autism. 
...
def show_results():
    ...
    if st.session_state.pred == 0:      # 0 - Severe Symptoms
        st.info(f"""
                The child is exhibiting several symptoms that are usually associated to ASD. 
                ASD screening by a medical professional is strongly recommended
                """)
        
    elif st.session_state.pred == 1:    # 1 - Mild Symptoms
        st.info(f"""
                The child is exhibiting some symptoms that may be associated to ASD. 
